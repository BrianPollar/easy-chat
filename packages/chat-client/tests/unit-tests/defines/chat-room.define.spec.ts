...
import { Chat, ChatRoom, ChatMsg } from '../../../src/defines/chat-room.define';
import { faker } from '@faker-js/faker';
import { createMockChatRoom, createMockChatMsg } from '../../../tests/mocks';
...
describe('ChatRoom', () => {
  ...
  it('update should add blocked members to local blocked list', () => {
    const newRoomVals = createMockChatRoom();
    newRoomVals.blocked = [faker.string.uuid(), faker.string.uuid()];
    const initialBlockedlong = instance.blocked.length;
    instance.update(newRoomVals, true);
    expect(instance.blocked).toBeDefined();
    expect(instance.blocked.length).toBeGreaterThan(0);
    expect(instance.blocked.length).toBe(newRoomVals.blocked.length + initialBlockedlong);
    expect(instance.blocked[initialBlockedlong]).toStrictEqual(newRoomVals.blocked[0]);
  });

  it('update should remove item from blocked list', () => {
    const newRoomVals = createMockChatRoom();
    newRoomVals.blocked = [faker.string.uuid(), faker.string.uuid()];
    instance.blocked = [...newRoomVals.blocked];
    const initialBlockedlong = instance.blocked.length;
    instance.update(newRoomVals, false);
    expect(instance.blocked).toBeDefined();
    expect(instance.blocked.length).toBeLessThan(initialBlockedlong);
  });
